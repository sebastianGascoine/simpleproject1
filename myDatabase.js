...
myDatabase.prototype.postStudent = function(student) {
  for (let i=0;i<this.students.length;i++) {
    ...
    this.students[studentIndex++] = new Student(student.id,student.name,student.debt);
    return true;
}
//similar to read
myDatabase.prototype.getStudent = function(id) {
  for (let i=0;i<this.students.length;i++) {
    if (this.students[i] && id == this.students[i].id)
        {
      return(new Student(this.students[i].id,this.students[i].name,this.students[i].debt));
        }
  }
    return null;
}
...
myDatabase.prototype.putStudent = function(student) {
  for (let i=0;i<this.students.length;i++) {
    if (this.students[i] && this.students[i].id == student.id) {
      this.students[i] = new Student(student.id,student.name,student.debt);
      return true;
    }
  }
  return false;
